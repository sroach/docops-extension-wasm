...
fn parse_metrics_card(body: &str) -> Result<MetricsCard, String> {
    ...
    if parts.len() > 1 {
        ...
        let lines: Vec<&str> = data_part.lines().map(|l| l.trim()).filter(|l| !l.is_empty()).collect();
        
        let mut start_idx = 0;
        if let Some(first_line) = lines.first() {
            if first_line.to_lowercase().contains("metric") && first_line.to_lowercase().contains("value") {
                start_idx = 1;
            }
        }
...
fn render_svg(card: &MetricsCard, use_dark: bool) -> String {
    ...
    let mut svg = format!(
        r##"<svg id="{id}" width="{total_width}" height="{total_height}" viewBox="0 0 {total_width} {total_height}" xmlns="http://www.w3.org/2000/svg" class="metrics-card-container premium{extra_class}">
    ...
    <rect width="100%" height="100%" fill="url(#bg-grad_{id})" rx="24"/>
    
    <g transform="translate({padding}, 60)">
        <text x="0" y="0" class="eyebrow_{id}">Executive Dashboard</text>
        <text x="0" y="36" class="title-text_{id}">{title}</text>
    </g>
    
    <g transform="translate({padding}, 136)">
"##,
        id = id,
    ...
    for (i, metric) in card.metrics.iter().enumerate() {
        ...
        let display_sublabel = if sublabel.contains('%') || sublabel.contains("Growth") || sublabel.contains("Improvement") {
            format!("↗ {}", sublabel)
        ...
        svg.push_str(&format!(
            r##"        <g transform="translate({x}, 0)" filter="url(#card-shadow_{id})">
            <rect width="{card_width}" height="{card_height}" rx="24" fill="var(--card-bg)" stroke="var(--card-stroke)" stroke-width="1.2"/>
            <rect x="20" y="20" width="50" height="6" rx="3" fill="url(#accent-{accent_idx}_{id})"/>
            <circle cx="{circle_x}" cy="36" r="24" fill="url(#accent-{accent_idx}_{id})" opacity="0.12"/>
            <circle cx="{circle_x}" cy="36" r="24" fill="url(#shine_{id})"/>
            <text x="20" y="76" class="metric-value_{id}">{value}</text>
            <text x="20" y="104" class="metric-label_{id}">{label}</text>
