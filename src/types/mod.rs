...
pub fn render(
    ...
    match viz_type {
        ...
        "timeline" => timeline::render(body, controls),
        "steps" => steps::render(body, controls),
        "release" | "releasestrategy" => release_strategy::render(body, controls),
        "metrics" | "metricscard" => metrics_card::render(body, controls),
        other => Err(format!(
            "unknown visualization type '{other}' — expected one of: bar, pieslice, badge, adr, gherkin, scorecard, button, quadrant, metrics, gauge, recipe, timeline, release, steps"
        )),
    }
}
